...
public class TaskContext<T extends TaskWrapper> extends Context {
    ...
    private final void stop() {
        ...
        try {
            ...
            log.error(e, "stop回调错误");
        }
        try {
            doStop();
        } catch (Exception e) {
            log.error(e, "doStop错误");
        }
    }
    ...
    public synchronized void kill() {
        if (stopSingle) {
            log.warn("已接收停止信号"); return;
        }
        stopSingle = true;
        log.info("接收停止信号, 将会等待正在运行的Task执行完毕, 就自动关闭");
        checkShouldStop();
    }
     ...
     * @param fn
     */
    public final void exec(Runnable fn) {
     ...
     * @param task
     */
    public final void forceStartTask(T task) { if (running.get()) startTask(task); }
...
//        }
     ...
     * @param task
     */
    public final TaskContext<T> inWaitingQueue(final T task) {
        if (task == null) {
            log.warn("添加Task为空, 忽略!");
            return this;
        }
     ...
     * @param task
     */
    protected void postInWaiting(T task) {}
    ...
    protected void doStop() {
        log.info("结束. 成功了 {} 个Task, 失败了 {} 个Task, 共执行 {} 毫秒",
                successTasksCount,
                failureTasksCount,
    ...
    protected void verifyBeforeStart() {
        if (waitingTasks.isEmpty()) log.warn("没有可执行的Task");
    }
    ...
    private Executor getExecutor() {
        if (executor == null) {
            ...
            log.info("创建一个私有的线程池. executor: {}", executor);
        }
        return executor;
    }
     ...
     * @return
     */
    public final TaskContext setExecutor(Executor executor, boolean shared) {
     ...
     * @param callback
     */
    public final void registerStopCallback(Consumer<TaskContext> callback) {
    ...
    public final TaskContext setKey(Object key) {
    ...
    public TaskContext attr(Object key, Object value) {
        attr(key, value);
        return this;
    }
...
//     * @return
//     */
//    public final <K> K getSpringBean(Class<K> clz) {
//        return getSpringCtx().getBean(clz);
//    }
//
//
//    public final ApplicationContext getSpringCtx() {
//        return springCtx;
//    }
//
//
//    public TaskContext setSpringCtx(ApplicationContext springCtx) {
...
//    }
    ...
    public TaskContext setExecutingTaskLimit(Integer executingTaskLimit) {
        if (executingTaskLimit == null) {
            log.warn("参数 executingTaskLimit is null, 使用默认值: " + this.executingTaskLimit);
            return this;
        }
